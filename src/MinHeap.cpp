...
sf::Vector2f MinHeap::getPosI(int i){
    int pos = i - (1 << lg2[i]);
...
MinHeap::MinHeap(): DataTypes(), numValue(0){
...
void MinHeap::checkFunctionFast(){
...
std::shared_ptr <Node> MinHeap::newNode(int k){
...
void MinHeap::draw(sf::RenderTarget& target, sf::RenderStates states) const{
...
void MinHeap::checkPress(sf::Vector2f mousePos){
